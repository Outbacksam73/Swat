...
export default function SwatConsole() {
  ...
  // Handle PWA Mobile Installation Event (guarded for SSR)
  useEffect(() => {
  ...
  const speak = (message) => {
    ...
    try {
      ...
      // silently ignore speech errors in unsupported environments
    }
  };
  ...
  const handleInstallApp = async () => {
    ...
    if (deferredPrompt && typeof deferredPrompt.prompt === "function") {
      ...
      // Friendly fallback for mobile browsers
      if (typeof window !== "undefined" && window.alert) {
        window.alert(
          "Mobile Install: open your browser menu (Safari/Chrome) and choose 'Add to Home Screen' or 'Install App'."
        );
      }
    }
  };
  ...
  const handlePurge = () => {
    ...
    setTimeout(() => {
      setChartData((prev) =>
        prev.map((item) => ({ ...item, threats: 0, trapped: 0, purged: item.threats }))
      );
      setIsPurging(false);
  ...
  const handleCreateApi = (e) => {
    e.preventDefault();
    // Basic validation
    if (!apiNameInput.trim() || !apiEndpointInput.trim()) return;
    const newEntry = { id: Date.now(), name: apiNameInput.trim(), endpoint: apiEndpointInput.trim() };
    setCustomApis((prev) => [newEntry, ...prev]);
    setApiNameInput("");
    setApiEndpointInput("");
    speak(`Custom API endpoint generated for ${newEntry.name}`);
  };
    ...
    <div className="min-h-screen bg-[#05080f] text-slate-200 font-mono flex flex-col justify-between">
      ...
      <header className="border-b border-cyan-500/20 bg-[#090d16]/80 backdrop-blur px-4 py-3 sticky top-0 z-50 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg">
            <ShieldAlert className="h-5 w-5 text-cyan-400" aria-hidden />
          </div>
          <div>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          >
            <Download className="h-3.5 w-3.5" /> <span>Mobile App</span>
          </button>
          ...
          <a
            ...
            aria-label="Subscription pass (PayPal)"
          ...
          >
            <CreditCard className="h-3.5 w-3.5" /> <span>Subscription Pass</span>
          </a>
          ...
          <button
            onClick={() => {
              ...
              if (nextState) speak("Voice synthesis active.");
            }}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#090d16] border-b border-cyan-500/20 p-4 space-y-3">
          ...
          >
            <Download className="h-4 w-4" /> <span>Download / Install App</span>
          </button>
          ...
          >
            <CreditCard className="h-4 w-4" /> <span>PayPal Access Pass</span>
          </a>
          ...
          <button
            ...
            aria-label="Toggle voice announcements (mobile)"
          >
            {voiceEnabled ? <Volume2 className="h-4 w-4 text-cyan-400" /> : <VolumeX className="h-4 w-4 text-slate-500" />}
            <span>Toggle Voice Announcements ({voiceEnabled ? "ON" : "OFF"})</span>
          </button>
        </div>
      )}
        ...
        <div className="bg-[#090d16] border border-slate-800 rounded-xl p-4 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xs font-bold text-slate-300 uppercase flex items-center gap-2">
              <Cpu className="h-4 w-4 text-cyan-400" /> <span>System Threat Timeline</span>
            </h2>
            <div className="flex gap-4 text-[10px]">
              <span className="flex items-center gap-1 text-rose-400">
                <span className="w-2 h-2 rounded-full bg-rose-500 inline-block" /> Detected
              </span>
              <span className="flex items-center gap-1 text-emerald-400">
                <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> Purged
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#090d16] border border-slate-800 rounded-xl p-4 space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-xs font-bold text-slate-300 uppercase flex items-center gap-2">
                <Radio className="h-4 w-4 text-cyan-400" /> <span>Cell Tower Radar</span>
              </h3>
              <button
          ...
          <div className="bg-[#090d16] border border-slate-800 rounded-xl p-4 space-y-4">
            <div className="border-b border-slate-800 pb-3">
              <h3 className="text-xs font-bold text-slate-300 uppercase flex items-center gap-2">
                <Terminal className="h-4 w-4 text-emerald-400" /> <span>Custom API Generator</span>
              </h3>
            </div>

            <form onSubmit={handleCreateApi} className="space-y-2">
              <label className="sr-only" htmlFor="apiName">API Identifier</label>
              <input
                id="apiName"
                type="text"
              ...
              <label className="sr-only" htmlFor="apiEndpoint">Endpoint Route</label>
              <input
                id="apiEndpoint"
                type="text"
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-semibold transition"
              >
